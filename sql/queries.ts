export const edgesQueryProperty = `
...
WITH RECURSIVE bfs AS (
    ...
    FROM
        bfs
    JOIN
        main.claims c ON bfs.dst = c.src
    WHERE
        bfs.level < $2
),
edges AS (
...
INSERT INTO user_subsets.claims (subset_id, src, dst, property)
SELECT $3, edges.src, edges.dst, edges.property FROM edges;
`;

export const nodesQueryProperty = `
WITH RECURSIVE bfs AS (
    SELECT
        src,
        dst,
...
INSERT INTO user_subsets.items (subset_id, id, label, description, level, x, y)
SELECT $3, nodes.id, nodes.label, nodes.description, nodes.level, random() AS x, random() AS y
FROM nodes;
`;
